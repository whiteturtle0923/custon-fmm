...
namespace Fargowiltas.Items.Summons.SwarmSummons.Thorium
{
    public class OverloadChampion : ModItem
    {
        ...
        public override bool UseItem(Player player)
        {
            ...
            if (Main.netMode == 2)
            {
                NetMessage.BroadcastChatMessage(NetworkText.FromLiteral("An ancient army has arrived!"), new Color(175, 75, 255));
            }
            else
            {
                Main.NewText("An ancient army has arrived!", 175, 75, 255);
            }
