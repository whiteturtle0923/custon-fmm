...
namespace Fargowiltas.Items.Summons
{
	public class BakeSale : ModItem
	{
		...
		public override bool UseItem(Player player)
		{
			...
			{
				npc.SetDefaults(i);
				if (npc.townNPC && NPC.TypeToHeadIndex(npc.type) >= 0 && !NPC.AnyNPCs(npc.type) && npc.modNPC.CanTownNPCSpawn(100, 2000000000))
				{
					NPC.NewNPC((int)player.position.X + Main.rand.Next(-200,200), (int)player.position.Y - 50, npc.type);
					Main.NewText("The " + npc.modNPC.Name + " has arrived!", 175, 75, 255);
				}
			}
