...
namespace Fargowiltas.Items.Summons
{
    public class Abeemination2 : ModItem
    {
        ...
        public override bool Shoot(Player player, ref Vector2 position, ref float speedX, ref float speedY, ref int type, ref int damage, ref float knockBack)
        {
            ...
            Projectile.NewProjectile(pos, Vector2.Zero, type, 0, 0, Main.myPlayer, NPCID.QueenBee);

            if (Main.netMode == 2)
            {
                NetMessage.BroadcastChatMessage(NetworkText.FromLiteral("Queen Bee has awoken!"), new Color(175, 75, 255));
            }
            else
            {
