	...
	public class LifeForce : ModItem
	{
		public override void SetStaticDefaults()
		{
			...
			Tooltip.SetDefault("'Rare is a living thing that dare disobey your will' \n" + 
								...
								"Summons a super buffed hornet and spider minion at no cost\n" +
		...
		public override void UpdateAccessory(Player player, bool hideVisual)
        {
			...
			if (player.whoAmI == Main.myPlayer)
            {
				if(player.FindBuffIndex(125) == -1 && soulcheck.bee == true)
				{
					...
					{
						Projectile.NewProjectile(player.Center.X, player.Center.Y, 0f, -1f, 373, 40, 2f, Main.myPlayer, 0f, 0f);
					}
				}
				...
				if(player.FindBuffIndex(133) == -1 && soulcheck.spid == true)
				{
					if (player.ownedProjectileCounts[390] < 1 && player.ownedProjectileCounts[391] < 1 && player.ownedProjectileCounts[392] < 1)
					{
						...
						Projectile.NewProjectile(player.Center.X, player.Center.Y, 0f, -1f, i, 60, 3f, Main.myPlayer, 0f, 0f);
					}
				}
				
				else if(soulcheck.spid == true)
				{
					Projectile.NewProjectile(player.Center.X, player.Center.Y, 0f, -1f, 392, 60, 3f, Main.myPlayer, 0f, 0f);
				}
            }
